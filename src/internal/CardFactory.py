...
class CardFactory:
    def __init__(self) -> None:
        ...
        if (
            self.Diamonds_texture.format == 0
            or self.Spades_texture.format == 0
            or self.Hearts_texture.format == 0
            or self.Clubs_texture.format == 0
        ):
            raise IOError("Failed to load texture")

    def spawn_card(
        self, suit: internal.Suit, rank: str | int, x: int, y: int
    ) -> internal.Card:
